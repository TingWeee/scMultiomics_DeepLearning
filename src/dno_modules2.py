...
def generate_palette(metadata, colName, colormap=cc.glasbey_dark):
...
def plotReloadedObj(filePath, refCol, palette=None, legendAnchor = (1.05, 0.5), figsize = (16, 14),
                    colormap=cc.glasbey_dark, saveplot = False):
        reloadedObj = dill.load(open(filePath, "rb"))
        metadata=reloadedObj.gene_protein.umap_df
        if palette == None: palette = generate_palette(metadata, refCol, colormap=colormap)
        ##
        ...
        for x in plot_names:
                ...
                sns.scatterplot(data = n.umap_df,x="UMAP1", y = "UMAP2",
                                hue = refCol,palette=palette,s = 4,ax=ax[i1, i2]).set(title = f'{x} score:{n.score}')
                ax[i1, i2].get_legend().remove()
                ind+=1
                print(x, n.score)
        handles, labels = plt.gca().get_legend_handles_labels()
        fig.legend(handles, labels,loc='center right',bbox_to_anchor=legendAnchor)
        if saveplot:
